...
// ===========================================================================
// fp8 (e4m3) QK^T gemm helper. Byte-identical load/smem/ldmatrix path to the
// int8 compute_int_qk (8-bit, k=32, same m16n8k32 fragment layout); the ONLY
// differences are the MMA atom (s8s8s32 -> f8f8f32) and the accumulator type
// (int32 -> float). The dequant (q_scale*k_scale) folds in downstream exactly
// as the int8 path does. Additive: attn_utils.cuh / int8 kernels untouched.
// PV path is unchanged (bf16/fp16 V) — fp8 only on QK, which is where the win
// and the K-outlier precision concern both live; V stays high-precision.
// ===========================================================================
...
__device__ __forceinline__ void compute_fp8_qk(const smem_t<swizzle_mode, stride> &smem_Q, const smem_t<swizzle_mode, stride> &smem_K, float RS[][num_tiles_k][8], uint32_t &offset_Q, uint32_t &offset_K)
{
...
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
        mma::mma_sync_m16n16k32_row_col_f8f8f32<mma::MMAMode::kInit>(RS[fq][fk], RQ[fq], RK);
      }
    }
    offset_K = smem_K.advance_offset_by_column<2>(offset_K - (num_tiles_k * 16 * stride), iter);
  }

  // following iterations, mma mode is kInplace
#pragma unroll
...
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
        mma::mma_sync_m16n16k32_row_col_f8f8f32<mma::MMAMode::kInplaceUpdate>(RS[fq][fk], RQ[fq], RK);
      }
    }
    offset_K = smem_K.advance_offset_by_column<2>(offset_K - (num_tiles_k * 16 * stride), iter);
  }
...
// for case when num_tiles_qk_inner = 1 (Q preloaded into RQ)
template <uint32_t num_warps_q, uint32_t num_warps_k,
          uint32_t num_tiles_q, uint32_t num_tiles_k, uint32_t num_tiles_qk_inner,
          SwizzleMode swizzle_mode, uint32_t stride>
__device__ __forceinline__ void compute_fp8_qk(const smem_t<swizzle_mode, stride> &smem_K, float RS[][num_tiles_k][8], uint32_t RQ[][4], uint32_t offset_K)
{
...
// Per-token dequant: multiply each score RS_f32[fq][fk][k] (raw fp8 QK^T sum)
// by q_scale[query_row] * k_scale[key_col]. The (row,col) <-> (fq,fk,k,lane)
// mapping is the SAME as apply_causal_mask: q_row = Q_idx + fq*16 + 8*((k%4)/2),
// k_col = K_idx + fk*16 + 8*(k/4) + (k%2). qsc[fq][0/1] are the two query-row
// scales a lane owns (loaded once; query rows are iter-invariant). ksc reloads
// per K-block (4 distinct col offsets {0,1,8,9}). Out-of-range cols -> 0 (the
// subsequent out-of-bound / causal mask overwrites them with -inf).
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void apply_pertoken_scale(
    float RS_f32[][num_tiles_k][8], const float qsc[][2],
    const float* __restrict__ ksbase, uint32_t K_idx_lane_base, uint32_t kv_len)
{
  float ksc[num_tiles_k][4];
#pragma unroll
  for (uint32_t fk = 0; fk < num_tiles_k; fk++)
  {
    uint32_t c = K_idx_lane_base + fk * 16;
    ksc[fk][0] = (c + 0 < kv_len) ? ksbase[c + 0] : 0.0f;
    ksc[fk][1] = (c + 1 < kv_len) ? ksbase[c + 1] : 0.0f;
    ksc[fk][2] = (c + 8 < kv_len) ? ksbase[c + 8] : 0.0f;
    ksc[fk][3] = (c + 9 < kv_len) ? ksbase[c + 9] : 0.0f;
  }
#pragma unroll
  for (uint32_t fq = 0; fq < num_tiles_q; fq++)
#pragma unroll
...
#pragma unroll
      for (uint32_t k = 0; k < 8; k++)
        RS_f32[fq][fk][k] *= qsc[fq][(k % 4) / 2] * ksc[fk][(k / 4) * 2 + (k % 2)];
}
...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypeSVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_mean=false, bool PER_TOKEN=false>
__global__ void qk_f8_sv_f16_attn_kernel(int8_t *__restrict__ Q, int8_t *__restrict__ K, half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      ...
                      float sm_scale)
{
  ...
  float RS[num_tiles_q][num_tiles_k][8];  // fp8 QK accumulates directly in fp32
...
#pragma unroll
  ...
  float q_scale = 0.0f, dequant_scale = 0.0f;
  // PER_TOKEN: per-(token,head) scales; qsc holds this lane's two query-row
  // scales (iter-invariant), ksbase is the per-token k_scale base for this head.
  float qsc[num_tiles_q][2];
  const float* __restrict__ pt_ksbase = nullptr;
  if constexpr (PER_TOKEN)
  {
    const float* __restrict__ qsbase =
        Q_scale + batch_id * num_qo_heads * qo_len + head_id * qo_len;
    pt_ksbase = K_scale + batch_id * (num_qo_heads / num_kv_groups) * kv_len
                + (head_id / num_kv_groups) * kv_len;
#pragma unroll
    for (uint32_t fq = 0; fq < num_tiles_q; fq++)
    {
      uint32_t r0 = Q_idx_lane_base + fq * 16;
      qsc[fq][0] = (r0 < qo_len) ? qsbase[r0] : 1.0f;
      qsc[fq][1] = (r0 + 8 < qo_len) ? qsbase[r0 + 8] : 1.0f;
    }
    // per-token scale is applied to RS directly; keep sm_scale = base scale.
    sm_scale = original_sm_scale;
  }
  else
  {
    q_scale = Q_scale[q_scale_idx];
    dequant_scale = q_scale * K_scale[k_scale_idx + 0 * k_scale_advance_offset];
...
#pragma unroll
  for (uint32_t iter = 1; iter < num_iterations - 1; iter++)
  {
    ...
    if constexpr (num_tiles_qk_inner == 1)
    {
      compute_fp8_qk<num_warps_q, num_warps_k, num_tiles_q, num_tiles_k, num_tiles_qk_inner, swizzle_mode_QK, QK_SMEM_STRIDE / PACK_SIZE_QK>(
...
#pragma unroll
    ...
    if constexpr (PER_TOKEN)
    {
      apply_pertoken_scale<num_tiles_q, num_tiles_k>(
          RS_f32, qsc, pt_ksbase, K_idx_lane_base, kv_len);
    }
...
#pragma unroll
        for (uint32_t k = 0; k < 8; k++)
        {
          RS_f32[fq][fk][k] = RS[fq][fk][k] * (PER_TOKEN ? 1.0f : dequant_scale);
        }
      }
    }
